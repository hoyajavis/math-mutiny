  ...
  return (
    <div className="flex flex-col min-h-screen p-4 md:p-6 overflow-x-hidden overflow-y-auto w-full relative">
      <motion.button
        ...
        className="absolute bottom-4 left-4 z-50 bg-[#14b8a6] text-cyan-50 font-black px-4 py-2 text-xs border-4 border-cyan-950 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] uppercase hidden md:block select-none"
      >
        EASY MODE
      </motion.button>
      {megaRocket && (
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-12 gap-4 w-full">
        <div className="relative z-50">
          <h1 
            ...
            className="text-[12vw] sm:text-5xl md:text-7xl font-black text-cyan-950 uppercase tracking-tighter border-2 md:border-4 border-cyan-950 bg-[#0891b2] px-2 md:px-4 py-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] md:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] cursor-pointer select-none origin-bottom-left flex max-w-full leading-none"
            style={{ transform: 'rotate(-1deg)' }}
          >
            {"DIVISION MUTINY!".split('').map((char, index) => {
        ...
        <div className="flex flex-row flex-wrap gap-2 md:gap-4 items-center mt-2 md:mt-0">
          <div className="bg-cyan-950 text-cyan-50 px-2 py-1 sm:px-4 sm:py-2 border-2 sm:border-4 border-cyan-50 -rotate-2 transform text-right sm:text-left">
            <p className="text-[10px] sm:text-xs font-bold text-[#0891b2] uppercase tracking-widest">AGENT</p>
            <p className="text-sm sm:text-xl font-black uppercase">{currentUser}</p>
          </div>
          <div className="border-2 sm:border-4 border-cyan-950 bg-cyan-50 p-1 sm:p-3 rotate-1 flex sm:block items-center gap-2">
          ...
          <div 
            ...
            className="border-2 sm:border-4 border-cyan-950 bg-[#14b8a6] p-1 sm:p-3 -rotate-1 text-cyan-50 cursor-pointer select-none hover:scale-105 active:scale-95 transition-transform flex sm:block items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full">
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            <button 
              ...
              className="group relative bg-cyan-50 border-4 border-cyan-950 p-8 hover:translate-x-1 hover:-translate-y-1 transition-transform cursor-pointer shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] flex flex-col items-start justify-between text-left"
            >
              <div className="absolute -top-3 -left-3 bg-[#0ea5e9] border-2 border-cyan-950 px-2 text-sm font-bold text-cyan-950">01</div>
              <h2 className="text-3xl font-black mb-2 uppercase text-cyan-950">Sequential</h2>
              <p className="text-sm leading-tight text-cyan-950">Climb the ladder from 1 to 12. No shortcuts, just pure multiplication muscle!</p>
            ...
            <button 
              ...
              className="group relative bg-cyan-50 border-4 border-cyan-950 p-8 hover:translate-x-1 hover:-translate-y-1 transition-transform cursor-pointer shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] flex flex-col items-start justify-between text-left"
            >
              <div className="absolute -top-3 -left-3 bg-[#0284c7] border-2 border-cyan-950 px-2 text-sm font-bold text-cyan-950">02</div>
              {randomHighScore > 0 && (
                <div className="absolute -top-4 -right-3 bg-[#0891b2] border-2 border-cyan-950 px-2 py-1 text-xs font-black transform rotate-6 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] uppercase">
                  BEST: {randomHighScore} XP
                </div>
              )}
              <h2 className="text-3xl font-black mb-2 uppercase text-cyan-950">Random</h2>
              <p className="text-sm leading-tight text-cyan-950">Total chaos! Numbers flying everywhere. Can you keep up with the madness?</p>
              <div className="text-4xl self-end mt-4 text-[#ff0000] animate-pulse">💥</div>
            </button>

            <button 
              onClick={() => setMode('challenge')}
              className="group relative bg-[#000] border-4 border-cyan-950 p-8 hover:translate-x-1 hover:-translate-y-1 transition-transform cursor-pointer shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] flex flex-col items-start justify-between text-left"
            >
              <div className="absolute -top-3 -left-3 bg-[#ff0000] border-2 border-cyan-50 text-cyan-50 px-2 text-sm font-bold">BOSS</div>
              {challengeHighScore > 0 && (
                <div className="absolute -top-4 -right-3 bg-[#0284c7] border-2 border-cyan-950 px-2 py-1 text-xs font-black transform -rotate-6 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-cyan-950 uppercase">
                  BEST: {challengeHighScore}s LEFT
                </div>
              )}
              <h2 className="text-3xl font-black mb-2 text-cyan-50 italic uppercase">Boss Fight</h2>
            ...
            <button 
              ...
              className="group relative bg-cyan-50 border-4 border-cyan-950 p-8 hover:translate-x-1 hover:-translate-y-1 transition-transform cursor-pointer shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] flex flex-col items-start justify-between text-left"
            >
              <div className="absolute -top-3 -left-3 bg-[#ffff00] border-2 border-cyan-950 px-2 text-sm font-bold text-cyan-950">TIPS</div>
              <h2 className="text-3xl font-black mb-2 uppercase text-cyan-950">Brain Hacks</h2>
              <p className="text-sm leading-tight text-cyan-950">Visual shortcuts, the 'Rule of 9s', and secret ways to never fail again.</p>
              <div className="text-4xl self-end mt-4">🧠</div>
            </button>
          </div>

          <div className="lg:col-span-4 flex flex-col items-center justify-center relative hidden lg:flex">
            <div className="bg-cyan-50 border-4 border-cyan-950 p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative rotate-3 mb-6">
              <p className="font-bold text-lg leading-tight uppercase">{botMsg}</p>
              <div className="absolute -bottom-4 left-6 w-4 h-4 bg-cyan-50 border-r-4 border-b-4 border-cyan-950 rotate-45"></div>
            </div>
            <div className="text-8xl mt-2 animate-bounce cursor-pointer select-none" onClick={handleBotClick}>🤖</div>
            <div className="text-center font-black uppercase tracking-widest bg-cyan-950 text-cyan-50 px-3 py-1 mt-4 rotate-[-2deg]">Math-Bot 9000</div>
          </div>
        </div>
      </div>
    </div>
